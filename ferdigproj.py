...
def display_leaderboard():
    try:
        with open(highscore_fil, encoding="utf-8") as fil:
            highscore_data = pandasForSortingCSV.read_csv(fil, header=None, names=["Name", "Score"])
            highscore_data.sort_values("Score", axis=0, ascending=False, inplace=True)

            for i, row in highscore_data.head(10).iterrows():
                skriv_tekst(f"{row['Name']}: {row['Score']} poeng", tekst_font_mindre, (0, 0, 0), 600, 200 + i * 50)

    except FileNotFoundError:
        skriv_tekst("Ingen highscores funnet", tekst_font_mindre, (0, 0, 0), 600, 400)
    except Exception:
        skriv_tekst("Feil ved lesing av highscores", tekst_font_mindre, (255, 0, 0), 600, 400)
    ...
    if gamemeny:
        ...
        if leaderboard:
            ...
            if higher_or_lower and tokk_feil:
                ...
                if enter_highscore:
                    legg_til_highscore(navn_input, antall_riktig)
                    higher_or_lower = False
                    gamemeny = True
                    navn_input_active = False
                    navn_input = "" 
